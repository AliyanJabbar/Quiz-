...
let apiLink: string; //declaring apiLink
...
let quiz = async () => {
  ...
  console.log(
    chalk.magenta(
      ...
      `${playerName.name} WELCOME TO THE QUIZ GAME `
    ),
    chalk.green("-".repeat(50))
  );
  ...
  console.log(
    chalk.bgRed("NOTE:"),
    chalk.red(
      `THERE WILL BE ${chalk.yellow("FIVE(5)")} QUESTIONS. EACH QUESTION WILL CONTAIN ${chalk.yellow("one(1)")} MARK!`
    )
  );
      ...
      let question = await inquirer.prompt(
        {
          ...
          choices: Options.map((val:any) => val)
        }
      )
